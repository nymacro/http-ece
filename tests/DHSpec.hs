...
spec = do
  describe "ECDH" $ do
    describe "Draft Data" $ do
      let --receiver
          privateKeyClient = UB64.decodeLenient "iCjNf8v4ox_g1rJuSs_gbNmYuUYx76ZRruQs_CHRzDg"
          publicKeyClient  = UB64.decodeLenient "BPM1w41cSD4BMeBTY0Fz9ryLM-LeM22Dvt0gaLRukf05rMhzFAvxVW_mipg5O0hkWad9ZWW0uMRO2Nrd32v8odQ"
          -- sender
          privateServer    = UB64.decodeLenient "W0cxgeHDZkR3uMQYAbVgF5swKQUAR7DgoTaaQVlA-Fg"
          publicKeyServer  = UB64.decodeLenient "BLsyIPbDn6bquEOwHaju2gj8kUVoflzTtPs_6fGoock_dwxi1BcgFtObPVnic4alcEucx8I6G8HmEZCJnAl36Zg"
          -- shared
          salt             = UB64.decodeLenient "5hpuYfxDzG6nSs9-EQuaBg"
          ciphertext       = UB64.decodeLenient "BmuHqRzdD4W1mibxglrPiRHZRSY49Dzdm6jHrWXzZrE"

      it "should decode receiver private key correctly" $ do
        CI.mk (B16.encode privateKeyClient) `shouldBe` CI.mk "D3332E1CF749C05AA776927EF348C650000596714F33B91205FB8921C451DBB4"

      it "should decode receiver private key correctly" $ do
        CI.mk (B16.encode publicKeyClient) `shouldBe` CI.mk "044C4B524E06F633FE96CC68FE73A0952522777F1C3F23B360EDF066BA9531AF08EB2F9D8003B83DE3AE3BEB9BEE0A5CB9B3395A1987B980176CC30B517E06A45B"

      it "should decode sender private key correctly" $ do
        CI.mk (B16.encode privateServer) `shouldBe` CI.mk "4E5C131EDD75E64538C4ACF50629179F4F3FB03AD5EB5BAD4FEBF437E8AC483E"

      it "should decode sender private key correctly" $ do
        CI.mk (B16.encode publicKeyServer) `shouldBe` CI.mk "045A327A3192C8DC76D27FEF82694DC04E9AD060AF85DD931C281BC3A440EA15613C3670B7C06021D31CFABA81A827CC5E39E26F77C070C05F3C25E4DF7157D08F"
      ...
      it "should be able to load ECDH public point" $ do
        loadPublicPoint publicKeyClient `shouldBe` Just (Point 110007014751775540525562297837608235252671747216779544670154428477160159313209
